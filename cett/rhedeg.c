#define PICTURE_FLICKER_SLOWDOWN    4
...
static void clear_my_can_go(void)
{
	been_there = 0;
}
static int my_can_go(Point p, Course c) {
	assert_course(c);
	/* See if we have  been at the exit */
	if(been_there)return 0;
	if(p.x == X/2 && p.y == 0) been_there = 1;
	/* We must not take a short path */
	if(p.y == Y/2 && (p.x > X/8 && p.x < X - X/3)) return 0;
	return 1;
}
...
//		get_path(point(17, 10 /* X/2,Y-1 */), N, my_can_go);
			...
			/* The monsters place */
	set(point(SAUCER_START_X - 0, SAUCER_START_Y - UPPER_SPACE + 1));
	set(point(SAUCER_START_X - 1 , SAUCER_START_Y - UPPER_SPACE + 1));
	set(point(SAUCER_START_X - 2 , SAUCER_START_Y - UPPER_SPACE + 1));
...
static void Monster_new(void) {
	me->x = SAUCER_START_X - 2; // Left to right
	me->y = (SAUCER_START_Y - UPPER_SPACE + 1) + UPPER_SPACE; // Downwards
	me->dx = -1;
	me->dy = -1;
	srand ( time(NULL) );
	me->cnt = 0;
...
static void Monster_move(int * px, int * py, int * pPic) {
	...
	*pPic = ((me->cnt)/PICTURE_FLICKER_SLOWDOWN)%3 + 2 + 1;
	if(me->y < UPPER_SPACE) {
		/* Above the maze */
		if(me->cnt%200 < 150) {
	...
	//	;
	} else if(me->cnt > 100 && me->cnt%2 == 0) {
		/* Navigate through the maze */
		// konstig impl. men jag orkar inte...
		int newx, newy, dx, dy;
		assert(me->y >= UPPER_SPACE);
		do {
			dx = rand()%3 - 1;
			dy = rand()%3 - 1;
			...
			newx = me->x + dx;
			newy = me->y + dy;
			/* repeat if we cannot go there or we try diagonally or we stand still or we go back */
		}while(!canGoAt(newx, newy) || (dy != 0 && dx != 0)  || (dy == 0 && dx == 0) || (newx == me->lastx && newy == me->lasty));
...
struct Level monster_rhedeg =
{
	IDB_BACKGROUND_rhedeg,
	IDB_MONSTER_rhedeg,
	IDB_OBSTACLE,
	Monster_new,
	Monster_delete,
